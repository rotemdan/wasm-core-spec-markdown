...
1. Try [instantiating](exec-instantiation) `module` in `store` with [external addresses](syntax-externaddr) `externaddr*` as imports:
   1. If it succeeds with a [module instance](syntax-moduleinst) `moduleinst`, then let `result` be `moduleinst`.
   2. Else, let `result` be `ERROR`.
2. Return the new store paired with `result`.

```text
module_instantiate(S, m, ev*) = (S', F.MODULE)   (if instantiate(S, m, ev*) ->* S'; F; ε)
module_instantiate(S, m, ev*) = (S', ERROR)        (otherwise, if instantiate(S, m, ev*) ->* S'; F; result)
```
...
4. For each `import_i` in `import*` and corresponding `externtype_i` in `externtype*`, do:
   1. Let `IMPORT nm_{i1} nm_{i2} xt_i` be the deconstruction of `import_i`.
   2. Let `result_i` be the triple `(nm_{i1}, nm_{i2}, externtype_i)`.
...
4. For each `export_i` in `export*` and corresponding `externtype'_i` in `externtype'*`, do:
   1. Let `EXPORT nm_i externidx_i` be the deconstruction of `export_i`.
   2. Let `result_i` be the pair `(nm_i, externtype'_i)`.
5. Return the concatenation of all `result_i`, in index order.
...
2. If there exists an `exportinst_i` in `moduleinst.MIEXPORTS` such that [name](syntax-name) `exportinst_i.NAME` equals `name`, then:
   1. Return the [external address](syntax-externaddr) `exportinst_i.ADDR`.
3. Else, return `ERROR`.
...
1. Let `deftype` be the [defined type](syntax-deftype) `S.FUNCS[a].FITYPE`.
2. Return `deftype`.
...
```text
func_type(S, a) = S.FUNCS[a].FITYPE
```
...
1. Try [invoking](exec-invocation) the function `funcaddr` in `store` with [values](syntax-val) `val*` as arguments:
   1. If it succeeds with [values](syntax-val) `v'*` as results, then let `result` be `v'*`.
   2. Else if the outcome is an exception with a thrown [exception](exec-throw_ref) `refexnaddr exnaddr` as the result, then let `result` be `exception exnaddr`.
   3. Else it has trapped, hence let `result` be `ERROR`.
2. Return the new store paired with `result`.

```text
func_invoke(S, a, v*) = (S', v'*)      (if invoke(S, a, v*) ->* S'; F; v'*)
func_invoke(S, a, v*) = (S', exception a')   (if invoke(S, a, v*) ->* S'; F; (refexnaddr a') throw_ref)
func_invoke(S, a, v*) = (S', ERROR)      (if invoke(S, a, v*) ->* S'; F; trap)
```
...
```text
exn_alloc(S, tagaddr, val*) = (S ⊕ { EXNS exninst }, |S.EXNS|)   (if exninst = { EITAG tagaddr, EIFIELDS val* })
```

#### `exn_tag(store, exnaddr) : tagaddr`

1. Let `exninst` be the [exception instance](syntax-exninst) `store.EXNS[exnaddr]`.
2. Return the [tag address](syntax-tagaddr) `exninst.EITAG`.

```text
exn_tag(S, a) = exninst.EITAG   (if exninst = S.EXNS[a])
```
...
```text
exn_read(S, a) = exninst.EIFIELDS   (if exninst = S.EXNS[a])
```
