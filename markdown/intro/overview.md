...
* These are integers and IEEE754 numbers, each in 32 and 64 bit width.
* 32-bit integers also serve as Booleans and as memory addresses.
* The usual operations on these types are available, including the full matrix of conversions between them. There is no distinction between signed and unsigned integer types.
...
* In addition to these basic number types, there is a single 128 bit wide vector type representing different types of packed data.
* The supported representations are four 32-bit, or two 64-bit IEEE754 numbers, or different widths of packed integer values, specifically two 64-bit integers, four 32-bit integers, eight 16-bit integers, or sixteen 8-bit integers.
