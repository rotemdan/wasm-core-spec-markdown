...
The recommended extension for files containing WebAssembly modules in binary format is "`.wasm`" and the recommended MediaType is "`application/wasm`".
...
* `B?` is an optional occurrence of `B`. (This is a shorthand for `B^n` where `n <= 1`.)
