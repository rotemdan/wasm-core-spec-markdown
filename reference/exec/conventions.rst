...
.. math::
   {\mathit{configuration}} \hookrightarrow {\mathit{configuration}}
...
To avoid unnecessary clutter, the store :math:`s` and the frame :math:`f` are often combined into a *state* :math:`z`, which is a pair :math:`(s ; f)`.
Moreover, :math:`z` is omitted from reduction rules that do not touch them.
...
In particular, :ref:`values <syntax-val>` are defined to coincide with :math:`\mathsf{const}` and :math:`\mathsf{ref}` instructions,
...
.. note::
   ...
   .. math::
      (\I32{.}\CONST~n_1)~(\I32{.}\CONST~n_2)~(\I32 {.} \ADD) \stepto (\I32{.}\CONST~(n_1 + n_2) \mathbin{\mathrm{mod}} {2^{32}})

   Per this rule, two :math:`\mathsf{const}` instructions and the :math:`\mathsf{add}` instruction itself are removed from the instruction stream and replaced with one new :math:`\mathsf{const}` instruction.
...
.. note::
   ...
   .. math::
      ...
      & & \hookrightarrow & (\F64{.}\CONST~q_1)~(\F64{.}\CONST~q_4)~(\F64{.}\CONST~q_3)~(\F64 {.} \ADD)~(\F64 {.} \MUL) \\
      & & \hookrightarrow & (\F64{.}\CONST~q_1)~(\F64{.}\CONST~q_5)~(\F64 {.} \MUL) \\
      & & \hookrightarrow & (\F64{.}\CONST~q_6) \\
      \end{array}
   

   where :math:`q_4 = {-q_2}` and :math:`q_5 = {-q_2} + q_3` and :math:`q_6 = q_1 \cdot ({-q_2} + q_3)`.
