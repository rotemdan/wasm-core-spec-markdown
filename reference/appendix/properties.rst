...
The :ref:`type use <syntax-typeuse>` :math:`(\REC {.} i)` is :ref:`valid <valid-typeuse>` if:
...
.. math::
   ...
   C \vdashtypeuse \REC {.} i : \OKtypeuse
   }
   \qquad
   \end{array}
...
The :ref:`heap type <syntax-heaptype>` :math:`\BOT` is always :ref:`valid <valid-heaptype>`.
...
.. math::
   ...
   C \vdashheaptype \BOT : \OKheaptype
   }
   \qquad
   \end{array}
...
The :ref:`value type <syntax-valtype>` :math:`\BOT` is always :ref:`valid <valid-valtype>`.
...
The :ref:`recursive type <syntax-rectype>` :math:`(\TREC~{{\subtype}^\ast})` is :ref:`valid <valid-rectype>` for :math:`i` if:
   ...
   * Or:
      ...
      * The :ref:`sub type <syntax-subtype>` :math:`{\subtype}_1` is :ref:`valid <valid-subtype>` for :math:`i`.

      * The :ref:`recursive type <syntax-rectype>` :math:`(\TREC~{{\subtype'}^\ast})` is :ref:`valid <valid-rectype>` for :math:`i + 1`.
...
.. math::
   ...
   C \vdashrectypeext \TREC~\epsilon : {\OKsubtypeext}{(i)}
   }
   \qquad
   \frac{
   C \vdashsubtypeext {\subtype}_1 : {\OKsubtypeext}{(i)}
    \qquad
   C \vdashrectypeext \TREC~{{\subtype}^\ast} : {\OKsubtypeext}{(i + 1)}
   }{
   C \vdashrectypeext \TREC~({\subtype}_1~{{\subtype}^\ast}) : {\OKsubtypeext}{(i)}
   }
   \qquad
   \end{array}
...
The :ref:`sub type <syntax-subtype>` :math:`(\TSUB~{\TFINAL^?}~{{\typeuse}^\ast}~{\comptype})` is :ref:`valid <valid-subtype>` for :math:`i` if:


   * The length of :math:`{{\typeuse}^\ast}` is less than or equal to :math:`1`.

   * For all :math:`{\typeuse}` in :math:`{{\typeuse}^\ast}`:

      * The :ref:`type use <syntax-typeuse>` :math:`{\typeuse}` is :ref:`valid <valid-typeuse>`.

      * :math:`{\typeuse} \prec i` is true.

      * The :ref:`sub type <syntax-subtype>` :math:`{{\unrollht}}_{C}({\typeuse})` is of the form :math:`(\TSUB~{{\typeuse'}^\ast}~{\comptype'})`.
...
.. math::
   ...
   {|{{\typeuse}^\ast}|} \leq 1
    \qquad
   (C \vdashtypeuse {\typeuse} : \OKtypeuse)^\ast
    \qquad
   ({\typeuse} \prec i)^\ast
    \\
   ({{\unrollht}}_{C}({\typeuse}) = \TSUB~{{\typeuse'}^\ast}~{\comptype'})^\ast
    \\
   ...
   C \vdashsubtypeext \TSUB~{\TFINAL^?}~{{\typeuse}^\ast}~{\comptype} : {\OKsubtypeext}{(i)}
   }
   \qquad
   \end{array}
...
.. math::
   \begin{array}[t]{@{}lcl@{}l@{}}
   {{\unrollht}}_{C}({\deftype}) & = & {\unrolldt}({\deftype}) \\
   {{\unrollht}}_{C}({\typeidx}) & = & {\unrolldt}(C{.}\CTYPES{}[{\typeidx}]) \\
   {{\unrollht}}_{C}(\REC {.} i) & = & C{.}\CRECS{}[i] \\[0.8ex]
   \REC {.} j \prec i & = & j < i \\
   {\typeuse} \prec i & = & \mathsf{true} & \quad \mbox{otherwise} \\
   \end{array}
...
The :ref:`defined type <syntax-deftype>` :math:`({\rectype} {.} i)` is :ref:`valid <valid-deftype>` if:


   * Let :math:`{C'}` be the same context as :math:`C`, but with the sub type sequence :math:`{{\subtype}^{n}}` prepended to the field :math:`\CRECS`.

   * Under the context :math:`{C'}`, the :ref:`recursive type <syntax-rectype>` :math:`{\rectype}` is :ref:`valid <valid-rectype>` for :math:`0`.

   * The :ref:`recursive type <syntax-rectype>` :math:`{\rectype}` is of the form :math:`(\TREC~{{\subtype}^{n}})`.
...
.. math::
   \begin{array}{@{}c@{}}\displaystyle
   \frac{
   C, \CRECS~{{\subtype}^{n}} \vdashrectypeext {\rectype} : {\OKsubtypeext}{(0)}
    \qquad
   {\rectype} = \TREC~{{\subtype}^{n}}
    \qquad
   i < n
   }{
   C \vdashdeftype {\rectype} {.} i : \OKdeftype
   }
   \qquad
   \end{array}
...
.. math::
   \begin{array}{@{}c@{}}\displaystyle
   \frac{
   C{.}\CRECS{}[i] = \TSUB~{\TFINAL^?}~(\TSTRUCT~{{\fieldtype}^\ast})
   }{
   C \vdashheaptypematch \REC {.} i \subheaptypematch \STRUCT
   }
   \\[3ex]\displaystyle
   \frac{
   C{.}\CRECS{}[i] = \TSUB~{\TFINAL^?}~(\TARRAY~{\fieldtype})
   }{
   C \vdashheaptypematch \REC {.} i \subheaptypematch \ARRAY
   }
   \\[3ex]\displaystyle
   \frac{
   C{.}\CRECS{}[i] = \TSUB~{\TFINAL^?}~(\TFUNC~{t_1^\ast} \Tarrow {t_2^\ast})
   }{
   C \vdashheaptypematch \REC {.} i \subheaptypematch \FUNCT
   }
   \\[3ex]\displaystyle
   \frac{
   C{.}\CRECS{}[i] = \TSUB~{\TFINAL^?}~{{\typeuse}^\ast}~{\mathit{ct}}
   }{
   C \vdashheaptypematch \REC {.} i \subheaptypematch {{\typeuse}^\ast}{}[j]
   }
   \qquad
   \end{array}
...
:ref:`element <syntax-eleminst>`,
type, or just :math:`\mathsf{ok}` in the case of
:ref:`data <syntax-datainst>`
...
.. note::
   The field :math:`C{.}\mathsf{refs}` from the resulting context is meant to be constructed to contain all :ref:`function indices <syntax-funcidx>` from the module.
   However, modules and their instances can contain more than :math:`{2^{32}}` functions
   (at most :math:`{2^{32}}` definitions plus :math:`{2^{32}}` imports),
   while the highest representable function index is :math:`{2^{32}} - 1`.
   The variable :math:`k` in the rule hence allows picking an upper limit for :math:`C{.}\mathsf{refs}` that is smaller than the total number of functions,
   in case that is necessary for :math:`C{.}\mathsf{refs}` to be syntactically well-formed.
   In practice, :math:`k = {\min}({|{{\funcaddr}^\ast}|}, {2^{32}})` always is the maximally permissive choice.
